...
class Image
{
    ...
    static function thumb($filename, $width = null, $height = null, $crop = true)
    {
        ...
        if($filename && is_file(($filename = Yii::getAlias('@webroot') . $filename)))
        {
            ...
            $name_string = md5( $filename . filemtime($filename) . (int)$width . (int)$height . (int)$crop );
            $thumbName = substr($name_string, 0, 2).'/'.substr($name_string, 2, 2).'/'.$name_string. '.' . $info['extension'];
        ...
        elseif(extension_loaded('imagick'))
        {
            ...
            if($height && !$crop) {
                ...
                } elseif ($width == 0)
                {
                    ...
                    if ($image->width > $image->height) $ratio = $width / $image->width;
                    else $ratio = $height / $image->height;

                    $newWidth = $ratio * $image->width;
                    $newHeight = $ratio * $image->height;
                }

                //die(print $newWidth.' - '.$newHeight);
